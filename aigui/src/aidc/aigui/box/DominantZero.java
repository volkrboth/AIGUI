...
/**
 * Calculates the dominant zero.
 * @author Volker Boos
 * 
 */
public class DominantZero extends AbstractBox 
{
	static final String DOMINANT_ZERO = "DominantZero";
	...
	{
		super("DominantZero");
		hmProps.put((DOMINANT_ZERO), "zero");
	}
	...
	protected JPanel createPanel() 
	{
		...
		setHints(getInfoIcon(), "Evaluate and display the dominant zero");
		return mainPanel;
	}
	...
	public int evaluateNotebookCommands(AbstractBox ab)
	{
		...
		try {
			invalidateEvalState();
			String command = hmProps.get(DOMINANT_ZERO) + "= Solve[Numerator[" + getAncestorProperty("Function", "") + "]==0, s]//Factor";
			String result = MathAnalog.evaluateToOutputForm(command, 300, true);
			String command2 = hmProps.get(DOMINANT_ZERO) + "n = " + hmProps.get(DOMINANT_ZERO) + " /.GetDesignPoint[" + getProperty("MatrixEquations", "") + "]";
				...
				public Object construct() {
					{
						if (evaluateNotebookCommands((AbstractBox) ab) >= 0) {
							//byte[] tabx = null;
							String command = "DisplayForm[" + hmProps.get(DOMINANT_ZERO) + "]";
							try {
								//tabx = MathAnalog.evaluateToTypeset(command, 0, false, true);
								DisplayWindow dw = new DisplayWindow("DominantZero (" + boxNumber + ") - symbolic solution ");
				...
				public Object construct() {
					{
						if (evaluateNotebookCommands((AbstractBox) ab) >= 0) {
							//byte[] tabx = null;
							String command = "DisplayForm[" + hmProps.get(DOMINANT_ZERO) + "n]";
							try {
								//tabx = MathAnalog.evaluateToTypeset(command, 0, false, true);
								DisplayWindow dw = new DisplayWindow("DominantZero (" + boxNumber + ") - numeric solution ");
								dw.setTypesetCommand( command, 0);
							} catch (MathLinkException e) {
								MathAnalog.notifyUser();
							}
	...
	protected void createNotebookCommands()
	{
		...
		String command0 = hmProps.get(DOMINANT_ZERO) + "= Solve[Numerator[" + getAncestorProperty("Function", "") + "]==0, s]//Factor";
		((DefaultNotebookCommandSet)nbCommands).addCommand(command0);
		
		String command1 = hmProps.get(DOMINANT_ZERO) + "n = " + hmProps.get(DOMINANT_ZERO) + " /.GetDesignPoint[" + getProperty("MatrixEquations", "") + "]";
